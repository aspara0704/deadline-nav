# Deadline Navi v0.7.4

「間に合うなら、まだ下道で。」

v0.7.4 は、v0.7.3 の候補上限を維持しつつ、**同一計算内で一度Matrix評価したICを回復探索・最終fallbackへ再送しない版**です。料金取得と走行中の軽量更新は変更しません。
...
ZIPは `dist/deadline-nav-v0.7.4.zip` に生成されます。既知ICの存在、同梱読み込み、旧キャッシュからの切替、異常データ、外部IC検索遮断を検証し、必要ファイル不足なら生成を止めます。APIキーも有料API呼び出しも不要です。
