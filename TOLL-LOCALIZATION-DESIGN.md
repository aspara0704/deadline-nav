...
初期スコープは **A: 普通車・ETC・通常料金（時間帯等の割引なし）** とする。

v0.8.0 の料金コアは、既知の料金距離と料率区分を入力に取る純粋関数に限定する。
道路グラフや料金区分が未確定の区間を推測して金額化しない。
複数料率区間を順序付きsegmentとして入力できるようにし、100km/200kmの長距離逓減を
走行順に適用する。結果は道路グラフ・例外網羅が完成するまでは `estimate` とする。


## v0.8.2 関西料金pilot

v0.8.1 の道路グラフ検証で、motorway_junction の一点だけをIC代表点として使うと、
久御山南IC→枚方東ICが約27kmの遠回りになることが判明した。ICは上下線・入口ランプを
含む「施設」として扱う必要があるため、v0.8.2では named junction 周辺のグラフ頂点を
access set として扱い、基準経路の距離が実道路のオーダーになることをCIで検証する。

第二京阪道路については、NEXCO西日本の公表資料で、ETC対距離化の基準として
大都市近郊区間相当の29.52円/kmと固定額250円が示されている一方、短区間などには
激変緩和・料金調整が存在する。したがってv0.8.2では:
- 第二京阪の基準式を local-formula / estimate として実装
- 明示的に公表されている現行例だけ exact override として保持
- estimate を exact としてユーザーに表示しない
- app.js の候補選定にはまだ接続しない
とする。
